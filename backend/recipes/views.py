...
from .serializers import (IngredientSerialiser, RecipeCreateSerializer,
                          RecipeSerializer, ShoppingListSerializer,
                          AddFavoriteRecipeSerializer, TagSerializer)
...
class FavoriteAPIView(APIView):
    ...
    def get(self, request, recipe_id):
        ...
        serializer = AddFavoriteRecipeSerializer(
...
class ShoppingViewSet(APIView):
    ...
    def delete(self, request, recipe_id):
        ...
        if count_of_deleted == 0:
            return Response('Такого рецепта в списке покупок нет.',
                            status=status.HTTP_400_BAD_REQUEST)
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class DownloadShoppingCart(APIView):
    ...
    def get(self, request):
        ...
        wishlist = ([f" {item} - {value['amount']} "
                     ...
                     for item, value in shopping_list.item()])
        wishlist.append('\n')
        today = date.today()
        wishlist.append(f"\n FoodGram, {today.year}")
